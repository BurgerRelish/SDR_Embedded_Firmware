...
void handleControlMessage(std::shared_ptr<sdr::App>&, std::shared_ptr<InterfaceMaster>&);
...
void handleInterfaceMessage(std::shared_ptr<sdr::App>& app, InterfaceRxMessage& message, ModuleAddressMap& module_map) {
    ...
    switch (countChar(message.message, '|')) {
        ...
        {        
            ...
            module -> add_reading(
                sdr::Reading(variables, app -> get_epoch_time())
            );
        }
        break;
...
void handleControlMessage(std::shared_ptr<sdr::App>& app, std::shared_ptr<InterfaceMaster>& interface) {
    ControlQueueMessage msg;
    
}

/**
 * @brief Transmits any relay state changes made to any sdr::Module to the requested module.
 * 
 * @param app 
 * @param interface 
 */
void sendStateChanges(std::shared_ptr<sdr::App>& app, std::shared_ptr<InterfaceMaster>& interface) {
